...
const api=async(path,opts={})=>{const requestOpts={cache:'no-store',headers:{'Content-Type':'application/json',...authHeaders()},...opts};const r=await fetch(path,requestOpts);if(r.status===401&&await refreshSession()){const r2=await fetch(path,{...requestOpts,headers:{'Content-Type':'application/json',...authHeaders(),...(opts.headers||{})}});return handleApi(r2)}if(r.status===401){window.location.href='/login';const err=new Error('Sessão expirada. Entre de novo.');err.status=401;throw err}return handleApi(r)};
async function handleApi(r){if(r.status===204)return null;const raw=await r.text();let d;try{d=raw?JSON.parse(raw):null}catch{d=null}if(!r.ok){const err=new Error(d?.detail||raw||`Erro HTTP ${r.status}`);err.status=r.status;err.payload=d;throw err}return d}
